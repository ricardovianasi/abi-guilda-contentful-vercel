...
export default function PostContent({ post }: PostContentProps) {
  ...
  const featuredImage = updatedPost.fields.featuredImage as Asset | undefined;
  const featuredImageUrl = featuredImage?.fields?.file?.url
    ? `https:${featuredImage.fields.file.url}`
    : undefined;

  const author = updatedPost.fields.author as TypeAuthor | undefined;
  ...
  return (
    <article className="container mx-auto px-4 py-12 max-w-4xl">
      ...
      <header className="mb-8">
        ...
        {updatedPost.fields.tags && updatedPost.fields.tags.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {updatedPost.fields.tags.map((tag) => (
              <span
                key={tag}
